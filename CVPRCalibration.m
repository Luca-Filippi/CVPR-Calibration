...
%----Definizione matrice V----%
      ...
      imageData(ii).V=[];
      for kk=1:length(XYpixel)
               imageData(ii).V=[imageData(ii).V;imageData(ii).v12; (imageData(ii).v11-imageData(ii).v22)];
      end
      [imageData(ii).U, imageData(ii).W, imageData(ii).S]=svd(imageData(ii).V);
       
%----Calcolo vettore b----%
     imageData(ii).b=imageData(ii).S(:,6);
...
%imposto direttamente B33=1 dato che K33=1
     ...
     %imageData(ii).L=chol(imageData(ii).B); Avvolte B non risulta essere 
     % definita positiva quindi non posso usare la funzione chol

%----Calcolo matrice K----%

    %imageData(ii).K=inv(imageData(ii).L'); dato che non posso calcolare L
    %con chol(B) questa istruzione non va bene
    au=1/((imageData(ii).B(1,2)*sqrt((1+imageData(ii).B(2,3)^2)/imageData(ii).B(2,2)))*...
        (1-imageData(ii).B(1,3)*imageData(ii).B(2,3)*sqrt((1+imageData(ii).B(2,3)^2)/imageData(ii).B(2,2))));
    av=sqrt((1+imageData(ii).B(2,3)^2)/imageData(ii).B(2,2));
    u0=-imageData(ii).B(1,3)*au;
    v0=-imageData(ii).B(2,3)*av;
    imageData(ii).K=[au 0 u0; 0 av v0; 0 0 1];

%----Calcolo matrice R e vettore t----%
    l=1/norm(inv(imageData(ii).K)*imageData(ii).h1); %l sarebbe lambda
    imageData(ii).r1=l*inv(imageData(ii).K)*imageData(ii).h1;
    imageData(ii).r2=l*inv(imageData(ii).K)*imageData(ii).h2;
    imageData(ii).r3=l*cross(imageData(ii).r1,imageData(ii).r2);

    imageData(ii).R=[imageData(ii).r1 imageData(ii).r2 imageData(ii).r3];

    imageData(ii).t=l*inv(imageData(ii).K)*imageData(ii).h3;
...
for ii=1:size(imagePoints,1)
    u=imagePoints(ii,1);
    v=imagePoints(ii,2);
    m1=[u; v; 1];%m1 corrisponde alla notazione di m' (m primo) del PDF
...
for ii=1:length(iimage) 
    ...
    proj=[ proj_hom(1,:)./proj_hom(3,:); proj_hom(2,:)./proj_hom(3,:);proj_hom(3,:)./proj_hom(3,:) ];%proj_hom(3,:)./proj_hom(3,:)
    ...
    homogeneous = [vX; vY; ones(1,length(vX))];
    proj_hom=imageData(ii).H*homogeneous;
    proj2=[ proj_hom(1,:)./proj_hom(3,:); proj_hom(2,:)./proj_hom(3,:);proj_hom(3,:)./proj_hom(3,:) ];

    fill3(proj2(1,:),proj2(2,:),0.9*proj2(3,:),'green','FaceAlpha',alpha);
    %plot3(proj2(1,:),proj2(2,:),0.9*proj2(3,:),'Color','red');
    hold on;
    pause(1);
end
